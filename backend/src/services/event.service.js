...
const updateEvent = async (eventId, updateData, adminUserId) => {
  try {
    ...
    if (start_date !== undefined) {
      validatedStartDate = start_date ? new Date(start_date) : event.start_date;
    }
    if (end_date !== undefined) {
      validatedEndDate = end_date ? new Date(end_date) : event.end_date;
    }
    // If both dates are present (either from input or existing event data), validate them
    if ((start_date !== undefined || end_date !== undefined) && validatedEndDate && validatedStartDate && validatedEndDate <= validatedStartDate) {
